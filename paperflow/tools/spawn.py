...
#: 识别）；中文全角括号 `（）` 仍是分隔符。
#: lookbehind `(?<![A-Za-z0-9_])`（review Important 修复）：任务文本是 LLM 拼装的，可能省略
#: 空格——路径前可以是空白、标点（：:(/[/反引号）、中文，但**不含英文单词字符**。这样
#: `Q1/Q2`/`8/10`/`a/b` 等散文斜杠（前接 word char）仍忽略，而「审阅草稿文件：/tmp/x」（全角
#: 冒号紧邻）、`(/tmp/x)`、`` `/tmp/x` ``、`审阅/tmp/x`（中文紧邻）都能识别。误判安全：
#: 中文/标点紧邻的假阳性 → 保守跳过 done 缓存 → 安全重跑；真路径漏判只剩「前接英文单词
#: 字符」这一窄缝（模板路径几乎不这么拼）。
...
def _task_has_path(task: str) -> bool:
    ...
    为路径（假阳性）→ 保守跳过 done 缓存 → 安全重跑。lookbehind 放宽为「前接非英文单词
    字符」（review Important 修复）：LLM 拼装的任务文本可能省略空格，路径前是标点（全角
    冒号/左括号/反引号）或中文时也能识别；`Q1/Q2`/`8/10` 等散文斜杠（前接 word char）仍
    忽略。真路径漏判只剩「前接英文单词字符」窄缝，模板路径几乎不这么拼。
    """
