...
记录四种事件，统一追加写入 ``audit_YYYYMMDD.jsonl``：

- ``tool_invoked``：工具调用（含被拦截 / JSON 解析失败 / 未知工具），携带决策、
  结果状态、错误详情、策略依据、审批结果与因果链；
...
调用链通过 contextvar 维护：``before`` 压栈当前 span，``after`` 弹栈；
子 agent 在 to_thread + asyncio.run 里自动继承父链，各并发任务持独立的
context 副本，互不串扰。每天一个文件、追加写入，写盘时按当天解析文件名，
...
class AuditMiddleware(SecurityMiddleware):
    ...
    async def after(self, ctx: ToolContext) -> None:
        ...
        # → ctx.span_id 为 None，则此处补一个；未压栈则不弹栈。
