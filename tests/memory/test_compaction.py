...
@pytest.mark.asyncio
async def test_sliding_window_evicts_old_inserts_summary():
...
@pytest.mark.asyncio
async def test_summarize_via_structured_output():
    ...
    class FakeStructured:
        async def extract(self, prompt, schema, fallback=None):
            ...
            return schema(task_overview="概述", current_state="状态",
                          important_discoveries="发现", next_steps="下一步",
                          context_to_preserve="保留")
    out = await run_compaction(messages, s, llm=None,
                               structured=FakeStructured())
...
@pytest.mark.asyncio
async def test_sliding_window_keeps_tool_pair_together():
    ...
    out = await run_compaction(messages, s, llm=None, structured=None,
                               summary_text="摘要")
...
@pytest.mark.asyncio
async def test_orphan_tool_dropped():
