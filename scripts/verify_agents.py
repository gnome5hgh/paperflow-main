...
副作用（如实声明）：
- searcher 会真打 arXiv/OpenAlex 公开 API
- qa-agent 的 RAG 检索会加载真实 bge 模型（首次下载权重 ~30MB）
- writer 会真写一篇笔记到配置的 vault note 目录（供检查）
- reviewer 读草稿（argv[2]，可复用 writer 写出的笔记）对照原文（argv[1]）
"""
import asyncio
import sys
...
async def _accept_confirm(cr):
    ...
    必须提供——writer 定稿的 WriteFileTool requires_confirm=True
...
async def main(pdf: str, draft: str | None) -> int:
    ...
    print("== 1. searcher（真实 arXiv/OpenAlex）==")
    print(await _make_agent(registry, "searcher", config).run(
        "搜索 2023 年异构图神经网络链路预测的论文，最多 3 篇"))

    print("== 2. qa-agent（RAG 检索，首次加载 bge 模型）==")
    print(await _make_agent(registry, "qa-agent", config).run(
        "circRNA 的调控机制是什么？"))

    print("== 3. writer（真写笔记到 vault note/）==")
    print(await _make_agent(registry, "writer", config).run(
        f"为 {pdf} 生成笔记"))

    print("== 4. reviewer（审稿 writer 产出的笔记）==")
    if draft:
