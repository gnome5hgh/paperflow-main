...
以及 SpawnSubAgentTool——searcher 用它派发 reviewer 子 agent，对候选论文逐篇核验
「年份/等级/相关性/可下载性」后产出通过清单。spawn 工具需要构造参数(agent_timeouts)，
故 make_tools 传已实例化的工具实例而非类;allowed_spawns 声明放行 reviewer。
"""
...
from paperflow.tools import (
    WebSearchTool, FetchPdfTool, GlobTool, GrepTool,
)
